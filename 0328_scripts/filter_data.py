...
def construct_eval_user_prompt(data_entry):
    ...
    prompt = (
        ...
        "ONLY evaluate the quality of the response provided. "
        "Provide a score (1-5) and a detailed explanation of your reasoning. "
        "Your response must follow this format that ONLY contains score: at front then follow by your_score number between 1 to 5:\n"
        "Example:\n score: your_score (1-5)"
...
def filter_jsonl(input_file, output_file, filter_rate, api_base, model_name, max_tokens=256, temperature=0.7, max_workers=10):
    """
    Reads the input JSONL file, evaluates each data entry concurrently using the LLM evaluation prompt,
    sorts the entries by their evaluation score (highest first), and writes out the top fraction
    (as defined by filter_rate) to the output JSONL file.
    """
    processed_items = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(process_item, data_item, api_base, model_name, max_tokens, temperature) 
...
def main():
    parser = argparse.ArgumentParser(description="Filter a JSONL file using LLM evaluation with concurrent requests.")
    ...
    parser.add_argument("--gpu", type=int, default=8, help="GPU index for the vLLM server.")
    parser.add_argument("--max_workers", type=int, default=30, help="Maximum number of concurrent requests.")
